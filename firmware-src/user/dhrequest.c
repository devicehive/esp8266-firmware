...
HTTP_REQUEST * ICACHE_FLASH_ATTR dhrequest_create_register() {
	char lenbuf[16];
	char jsonbuf[sizeof(REGISTER_JSON) - 2 + mDeviceKeyLen];
	const unsigned int jsonbuflen = os_sprintf(jsonbuf, REGISTER_JSON, mDeviceKey);
	const unsigned int lenbuflen = os_sprintf(lenbuf, "%d", jsonbuflen);
	const unsigned int len = sizeof(HTTP_REQUEST_BODY) - 20 + 3 + mServerLen + 8 + mDeviceIdLen + mDeviceIdLen + mDeviceKeyLen + lenbuflen + jsonbuflen;
	HTTP_REQUEST *res = (HTTP_REQUEST *)os_malloc(len + sizeof(HTTP_REQUEST));
	if(res == NULL) {
		dhdebug("Error: no memory for register request");
		return NULL;
	}
	res->len = os_sprintf(res->body, HTTP_REQUEST_BODY, "PUT", mServer, "/device/", mDeviceId, "", "", mDeviceId, mDeviceKey, lenbuf, jsonbuf);
...
HTTP_REQUEST *dhrequest_update_poll(HTTP_REQUEST *old, const char *timestamp) {
	...
	const unsigned int len = sizeof(HTTP_REQUEST_BODY) - 20 + 3 + mServerLen + 8 + mDeviceIdLen + 24 + timestamplength + mDeviceIdLen + mDeviceKeyLen + 1;
	HTTP_REQUEST *next;
	if(old) {
		if(old->len + 1 == len) {
	...
	res->len = os_sprintf(res->body, HTTP_REQUEST_BODY, "GET", mServer, "/device/", mDeviceId, "/command/poll?timestamp=", timestamp, mDeviceId, mDeviceKey, "0", "");
...
HTTP_REQUEST * ICACHE_FLASH_ATTR dhrequest_create_update(int commandId, const char *status, const char *result) {
	...
	char lenbuf[16];
	const unsigned int lenbuflen = os_sprintf(lenbuf, "%d", jsonbuflen);
	const unsigned int len = sizeof(HTTP_REQUEST_BODY) - 20 + 3 + mServerLen + 8 + mDeviceIdLen + 9 + idbuflen + mDeviceIdLen + mDeviceKeyLen + lenbuflen + jsonbuflen;
	HTTP_REQUEST *res = (HTTP_REQUEST *)os_malloc(len + sizeof(HTTP_REQUEST));
	if(res == NULL) {
		dhdebug("Error: no memory for update request");
		return NULL;
	}
	res->len = os_sprintf(res->body, HTTP_REQUEST_BODY, "PUT", mServer, "/device/", mDeviceId, "/command/", idbuf, mDeviceId, mDeviceKey, lenbuf, jsonbuf);
...
HTTP_REQUEST * ICACHE_FLASH_ATTR dhrequest_create_info() {
	const int len = sizeof(HTTP_REQUEST_BODY) - 20 + 3 + mServerLen + 5 + mDeviceIdLen + mDeviceKeyLen +1;
	HTTP_REQUEST *res = (HTTP_REQUEST *)os_malloc(len + sizeof(HTTP_REQUEST));
	if(res == NULL) {
		dhdebug("Error: no memory for info request");
		return NULL;
	}
	res->len = os_sprintf(res->body, HTTP_REQUEST_BODY, "GET", mServer, "/info", "", "", "", mDeviceId, mDeviceKey, "0", "");
...
HTTP_REQUEST * ICACHE_FLASH_ATTR dhrequest_create_notification(const char *name, const char *parameters) {
	...
	char lenbuf[16];
	const unsigned int lenbuflen = os_sprintf(lenbuf, "%d", jsonbuflen);
	const unsigned int len = sizeof(HTTP_REQUEST_BODY) - 20 + 4 + mServerLen + 8 + mDeviceIdLen + 13 + mDeviceIdLen + mDeviceKeyLen + lenbuflen + jsonbuflen;
	HTTP_REQUEST *res = (HTTP_REQUEST *)os_malloc(len + sizeof(HTTP_REQUEST));
	if(res == NULL) {
		dhdebug("Error: no memory for notification");
		return NULL;
	}
	res->len = os_sprintf(res->body, HTTP_REQUEST_BODY, "POST", mServer, "/device/", mDeviceId, "/notification", "", mDeviceId, mDeviceKey, lenbuf, jsonbuf);
