...
LOCAL void ICACHE_FLASH_ATTR dhspi_reinit() {
	...
	WRITE_PERI_REG(SPI_USER1, ((7 & SPI_USR_MOSI_BITLEN) << SPI_USR_MOSI_BITLEN_S) |
...
int ICACHE_FLASH_ATTR dhspi_set_cs_pin(unsigned int cs_pin) {
	if(cs_pin == 12 || cs_pin == 13 || cs_pin == 14 || cs_pin > DHGPIO_MAXGPIONUM || ((1 << cs_pin) & DHGPIO_SUITABLE_PINS) == 0)
		return 0;
	mSPICSPin = cs_pin;
	return 1;
}

void ICACHE_FLASH_ATTR dhspi_write(const char *buf, unsigned int len, int disable_cs) {
	dhspi_reinit();
