...
DH_I2C_Status ICACHE_FLASH_ATTR ina219_read(int sda, int scl, float *voltage, float *current, float *power) {
	char buf[12];
	...
	char i;
	for(i = 1; i < 5; i++) {
		if((status = dh_i2c_write(mAddress, &i, 1, 0)) != DH_I2C_OK) {
...
DH_I2C_Status ICACHE_FLASH_ATTR ina219_set_shunt(float resistance) {
	if(mResistance <= 0.0f)
